...
Let $R$ be a rational number which is not a perfect $p$-th root and let $p$ be a prime number so we don't have to further reduce the expression (composite numbers can always be factored into primes which can subsequently 
be considered separately by nesting the radical expressions). Let $\alpha = \sqrt[p]{R}$ and
...
\be  f(\alpha) = c_0 + c_1 \alpha  + \dots +  c_{p-1} \alpha^{p-1},\quad c_i \in F, i = 0,1,2\hdots, p-1. \label{AlphaBasis} \ee

A rational expression in $V$ takes the form \( P(\alpha) / Q(\alpha) \), where 
\begin{eqnarray*}
P(\alpha) &=& a_0 + a_1 \alpha  + \dots +  a_{m} \alpha^{m} \cr
Q(\alpha) &=& b_0 + b_1 \alpha  + \dots +  b_{n} \alpha^{n},
\end{eqnarray*}where $a_i \in F$ for $i = 0, 1, 2, \hdots, ,m$  and $b_j = 0, 1, 2, \hdots n$. \\

Such a rational expression can be brought back into the form of   \(  c_0 + c_1 \alpha  + \dots +  c_{p-1} \alpha^{p-1} \), by first using Eq. (\ref{AlphaBasis}) for any terms with degree greater than $p$ in either the numerator or denominator and by rationalizing the denominator by multiplying $Q(\alpha)$
by all of the conjugate values using the $p$-roots of unity $\omega$: $\omega^p = 1$.

We put this in the form of:
\begin{theorem}
Any algebraic function can be put in the form
\[ y = c_0 + c_1 \alpha + c_2 \alpha^2 + \hdots + c_{p-1} \alpha^{p-1}, \]
for $p$ a prime and $c_0, c_1, \hdots c_{p-1}, R$ are algebraic functions of lower order than y.
\end{theorem}

\begin{proof}

We start by showing that it is possible to clear the denominator of terms that depend only on rational quantities and a free of roots of unity. To proceed we multiply the numerator and denominator by the conjugate values of $Q(\alpha)$ over the $p$ roots of unity: 
...
It is certainly true the the final expression for $R$ is symmetric in the roots of the polynomial $Q(x)$ and hence by the theorem on Symmetric Functions can be expressed in terms of the coefficients of $Q(x)$ and hence is rational. 
...
\begin{align*}
f(x)  &= x^{p} - d\\
Q(x) &= b_{n} \alpha^{n} + b_{n-1}\alpha^{n-1} + \dots + b_1 \alpha    + b_0  , 
\end{align*} 
where $R$ is not a $p$-th power of an element in the field of coefficients. 
$f(x)$ and $Q(x)$ will have a common root if the Sylvester Resultant formed by these two functions vanishes. 
The roots of $f(x) = 0$ take the form $z_k = \omega^k \sqrt[p]{R}$ and we're assuming that $d^{1\over p}$ is an irrationality in the domain containing the coefficients. 
Also with respect to Abel's rationalization method 
we need to show that 
\be \prod_{k=0}^{p-1}\, Q(z_k) = G(z^p)  = G(d)  \mbox{  where $G(d)$ is rational in the domain of the coefficients of  }  Q(x).\ee
...
final polynomial expression outputted by the determinant can only contain powers of combinations of the terms in the Sylvester matrix. 
...
%\begin{eqnarray*}
%x^p - d &=& \prod_{i=0}^{p-1} (x - \omega^i \sqrt[p]{R}) \mbox{ therefore} \cr
...
However, when the final element wraps around to the beginning, it is multiplied by a specific complex scalar $R$. \\
...
Let \(\alpha = \sqrt[n]{R}\) be an algebraic radical of degree \(n\). 
...
{\bf Eigenvalues:} While a standard circulant matrix uses pure Fourier modes as eigenvectors, a $R$-circulant matrix's eigenvectors account for the scalar shift $R$. 
...
Since the left-hand side involves the determinant of an $n \times n$ matrix whose elements are free of \(\omega \) and \(\alpha  =  \sqrt[p]{R} \), therefore expanding the product on the right-hand side eliminates all instances of \(\omega \) and \(\alpha  \).
This determinant is guaranteed to be a purely rational number if all \(a_{j}\) and $R$ are rational. 
...
Mathematica Basis &  \makecell{Linear operator on the field basis \\ $\{1, \alpha, \dots, \alpha^{p-1}\} $}    & \makecell{Elimination tool for two \\ distinct  polynomial rings} \\ \hline
Intrinsic Rule & \makecell{Radical Reduction $x^p = R$ \\ built in to the ``twist''}& \makecell{Radical Reduction explicitly handled \\ by inserting $-R$ into the rows }\\ \hline
