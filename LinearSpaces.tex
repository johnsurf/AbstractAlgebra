...
Any set of numbers $x^1, x^2, ..., x^n$ which satisfy Eq. (\ref{HomogenousSystem}) are now taken to be the components of an $n$-dimensional vector $\r = \{x^1, x^2, ..., x^n\}$. 
...
\begin{theorem}
All the solutions of a non-homogenous system of equations (\ref{LinearSystem}) are of the form $z = \r + \eta$, where $\r$ is a fixed solution of the non-homogenous system (\ref{LinearSystem})
...
$$ \a_i \cdot \r = 0,$$ where we set $\r = \{x^1, x^2, ..., x^n\}$.
...
$$\a_k \cdot \r =   \lambda^k_1 (\a_1\cdot \r) + \lambda^k_2 (\a_2 \cdot \r)+ ...  + \lambda^k_s (\a_s\cdot \r).$$ Since $\a_i \cdot \r = 0$ for $i = 1,2, ..., s$ the right hand side 
of this last equation becomes $0$, so that $$\a_k \cdot \r = 0, \quad k = 1,2, ..., m,$$ and in particular for $k = s+1, s+2, ..., m$ as was to be proved.\\
...
$$\r = \{ x^1, x^2, ..., x^n\}$$ then the equations 

\begin{equation}\a_i \cdot \r = 0, \quad i = 1, 2, ..., p,\label{10}\end{equation}
...
\begin{equation}\b_1 \cdot \r = 0, ~~\b_2 \cdot \r = 0, ~~..., ~~\b_{n-p} \cdot \r = 0.\label{11}\end{equation}
