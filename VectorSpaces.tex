...
\begin{theorem}[Grassmann dimension formula]
If $M$ and $N$ are subspaces of a finite dimensional vector space $V$, then 
\[ \dim(M + N) = \dim M + \dim N - \dim (M\cap N).\] 
\label{Grassmann}\end{theorem}

\begin{proof}
Let $\dim M = \mu$, $\dim N = \nu$, $\dim (M\cap N) = \rho$, which are all finite. Take $\z_1, \hdots, \z_\rho$ as the basis of $M\cap N$. Since $M\cap N$ is a subspace of both $M$ and $N$, it can be extended as follows to a basis for both $M$ and $N$: 
\begin{eqnarray*}
\z_1, \hdots, \z_\rho, \quad \x_1, \hdots, \x_{\mu - \rho} &\mbox{ of }& M \mbox{ and }\cr
\z_1, \hdots, \z_\rho, \quad \y_1, \hdots, \y_{\nu - \rho} &\mbox{ of }& N
\end{eqnarray*}
We now prove that 
\[ \z_1, \hdots, \z_\rho, \quad \x_1, \hdots \x_{\mu - \rho}, \quad \y_1, \hdots, \y_{\nu - \rho} \] is a basis of $M + N$. 

Any element of $M$ is a linear combination of $\z_1, \hdots, \z_\rho, \x_1, \hdots, \x_{\mu - \rho}$ and any element of $N$ is a linear combination of $\z_1, \hdots, \z_\rho, \y_1, \hdots, \y_{\nu - \rho}$. Therefore, any element of $M + N$ is a linear combination of the $\z$'s, $\x$'s and $\y$'s, and so this set spans $M + N$. 

Now suppose that \[ \sum_{i = 1}^\rho\, \alpha_i \z_i + \sum_{j = 1}^{\mu - \rho}\, \beta_j \x_i + \sum_{k = 1}^{\nu - \rho}\, \gamma_k \y_k = \o.\] Then   
\[      \sum_{k = 1}^{\nu - \rho}\, \gamma_k \y_k =  - \sum_{i = 1}^\rho\, \alpha_i \z_i - \sum_{j = 1}^{\mu - \rho}\, \beta_j \x_i  \in M,\] and also in $N$, and so is in $M\cap N$.
Therefore  $\displaystyle \sum_{k = 1}^{\nu - \rho}\, \gamma_k \y_k  = \sum_{i=1}^\rho \delta_i \z_i $ since $\z_1, \hdots, \z_\rho$ are a basis for $M\cap N$. But the $\y_i$'s and $\z_i$'s together form a linearly independent set, being a basis of $N$, and therefore all the $\gamma_i$'s are zero. Similarly all the $\beta_i$'s are zero, and so $\displaystyle \sum_{i = 1}^\rho \, \alpha_i \z_i = \o$ and all the $\alpha_i$'s are zero because $\z_1,\hdots,\z_\rho$ are linearly independent. \\

Therefore $\z_1, \hdots, \z_\rho, \quad \x_1, \hdots, \x_{\mu - \rho}, \quad \y_1, \hdots, \y_{\nu - \rho}$ are linearly independent and hence form a basis of $M + N$ and we have
\[ \dim(M+N) = \rho + \mu - \rho + \nu - \rho = \mu + \nu - \rho, \mbox{ or}\] 
\[ \dim(M + N) = \dim M + \dim N - \dim (M\cap N). \qed\]  
\end{proof}

Thm (\ref{Grassmann}) is basically Inclusion-Exclusion for vector spaces. The main reason this is the case is that it is possible to build a sequence of vector spaces where the Euler characteristic (alternating sum of dimensions) vanishes, which is directly related to the Inclusion-Exclusion formula. The two principles are the same idea showing up in different contexts: counting elements versus measuring linear independence.  