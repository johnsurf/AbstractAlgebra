...
\begin{definition}
$A$ is called the object space of the mapping, $B$ is the image space, $a$ is the object whose image is $b$ and we say that we have a mapping of $A$ into $B$. If the mapping is denoted by $\theta$, we write 
$\theta : A \goes B$, and we say that $a\theta = b$ or $a \goes b$. 
\end{definition}

In any homomorphism between a group $G$ and another group $H$ the neutral element $e$ of $G$ maps into the neutral element $f$ of $H$. The set of elements which form a subset of $G$ which map into $f$ is called the {\twelveit kernel} of the homomorphism. 

\begin{theorem}
The kernel of any homomorphism is a subgroup of $G$. \label{kernel}
\end{theorem}

\begin{theorem}
The image $G\theta$ is a subgroup of $H$.  \label{image}
\end{theorem}